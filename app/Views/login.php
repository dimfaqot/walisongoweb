...
<head>
    ...
    <style>
        .active {
            background-color: <?= temaroutes('mid_light'); ?>;
            color: <?= temaroutes('dark'); ?>;
        ...
        .checkmark {
            ...
            margin: 10% auto;
            box-shadow: inset 0px 0px 0px <?= colorroutes('success main'); ?>;
        ...
        @keyframes fill {
            100% {
                box-shadow: inset 0px 0px 0px 30px <?= colorroutes('success main'); ?>
            }
        }
        ...
        .load span {
            ...
            height: 20px;
            width: 20px;
            background: <?= colorroutes('primary dark'); ?>;
            border-radius: 50%;
        ...
        .load span:nth-child(4) {
            ...
            background: <?= colorroutes('info main'); ?>;
        }

        .boxlist {
            background-color: <?= temaroutes('light'); ?>;
            border: 1px solid <?= temaroutes('mid_light'); ?>;
            border-radius: 10px;
            padding: 10px;
        }

        .bglist {
            background-color: <?= temaroutes('secondary'); ?>;
            border-radius: 10px;
            color: <?= temaroutes('dark'); ?>;
            padding: 5px 10px 5px 10px;
            font-size: <?= temaroutes('font_size'); ?>;
            font-weight: 700;
        }

        .form-check-input:checked {
            background-color: <?= temaroutes('main'); ?>;
            border-color: <?= temaroutes('mid_dark'); ?>;
        }
...
<?php if (session()->getFlashdata('gagal')) : ?>
    ...
    <div class="gagal blur" style="border-radius: 10px;">
        <div class="middlecenter">
            <div class="d-flex justify-content-between bg-danger px-1" style="border-radius: 10px;width:300px; color:<?= colorroutes('danger light'); ?>;font-size:<?= temaroutes('font_size'); ?>;">

                <div class="toast-body p-2" style="border-radius: 10px; font-size:<?= temaroutes('font_size'); ?>;">
                    <?= session()->getFlashdata('gagal'); ?>
                </div>
                <div>
                    <button type="button" class="btn btn-sm m-auto btnclose" style="color: <?= colorroutes('danger light'); ?>;"><i class="fa fa-times-circle"></i></button>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="<?= base_url('login'); ?>/auth" method="post">
                                        <?= csrf_field() ?>
                                        <div class="input-group input-group-sm mb-3 select" style="border-color:<?= temaroutes('main'); ?>">
                                            <span class="input-group-text" style="width:100px;font-size:<?= temaroutes('font_size'); ?>; background-color:<?= temaroutes('secondary'); ?>;border-color:<?= temaroutes('mid_light'); ?>">Username</span>
                                            <input style="font-size:<?= temaroutes('font_size'); ?>; border-color:<?= temaroutes('mid_light'); ?>;" name="username" type="text" class="form-control" placeholder="Username" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm" autofocus>
                                        </div>
                                        <div class="input-group input-group-sm mb-3 select" style="border-color:<?= temaroutes('main'); ?>">
                                            <span class="input-group-text" style="width:100px;font-size:<?= temaroutes('font_size'); ?>; background-color:<?= temaroutes('secondary'); ?>;border-color:<?= temaroutes('mid_light'); ?>">Password</span>
                                            <input style="font-size:<?= temaroutes('font_size'); ?>; border-color:<?= temaroutes('mid_light'); ?>;" name="password" type="password" class="form-control" placeholder="Password" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm">
                                        </div>

                                        <div class="d-grid gap-2 d-flex justify-content-center">
                                            <button type="submit" class="btn" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem; background-color:<?= temaroutes('main'); ?>; color:<?= temaroutes('light'); ?>">
